한 줄 정리
> **Java Bean**은 “자바 객체를 일정한 규약에 맞게 만든 클래스”이다.
...
이 경우 Java Beans 관점에서는 필드명이 `username`이어도 프로퍼티 이름은 `name`으로 볼 수 있다.

Java Bean은 주로 다음 용도로 많이 쓰인다.
...
Java Bean의 핵심은 **객체를 표준화된 방식으로 읽고 쓸 수 있게 하는 규약**이다.